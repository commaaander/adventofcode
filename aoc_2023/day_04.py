...
--- Day 4: Scratchcards ---
The gondola takes you up. Strangely, though, the ground doesn't seem to be coming with you; you're not climbing a mountain. As
the circle of Snow Island recedes below you, an entire new landmass suddenly appears above you! The gondola carries you to the
...
borrow my boat and you can go visit the gardener. I got all these scratchcards as a gift, but I can't figure out what I've won."
...
In the above example, card 1 has five winning numbers (41, 48, 83, 86, and 17) and eight numbers you have (83, 86, 6, 31, 17, 9,
48, and 53). Of the numbers you have, four of them (48, 83, 17, and 86) are winning numbers! That means card 1 is worth 8 points
...
Copies of scratchcards are scored like normal scratchcards and have the same card number as the card they copied. So, if you win
...
4 instances of card 3, 8 instances of card 4, 14 instances of card 5, and 1 instance of card 6. In total, this example pile of
...
def part_one(**kwargs: Dict[str, Any]) -> Tuple[str, str]:
    ...
    cards = parse_raw_data(kwargs["raw_data"])
    for card in cards.values():
        LOG.debug(f"part_one()::{card=}")
        if card["match_count"] > 0:
            answer += 2 ** (card["match_count"] - 1)
...
def part_two(**kwargs: Dict[str, Any]) -> Tuple[str, str]:
    ...
    for i in range(len(cards)):
        ...
        LOG.debug(f"part_two()::card_number={i + 1}, {card=}")
        if card["match_count"] > 0:
            for j in range(card["match_count"]):
                cards[i + j + 2]["count"] += card["count"]
    LOG.debug(f"part_two()::{cards=}")
    return questions[1], sum(card["count"] for card in cards.values())


def parse_raw_data(raw_data: str) -> Dict[int, Dict[str, int | List[int]]]:
    parsed_data = {}
    for line in raw_data.splitlines():
        match = re.match(r"Card (.*): (.*) \| (.*)", line)
        numbers = [int(i) for i in match.group(2).split()]
        winning_numbers = [int(i) for i in match.group(3).split()]
        number_matches = set(numbers) & set(winning_numbers)
        parsed_data[int(match.group(1))] = {
            "count": 1,
            "numbers": numbers,
            "winning_numbers": winning_numbers,
            "matches": number_matches,
            "match_count": len(number_matches),
        }
