...
def passportValidation(passports):
    ...
    requiredFields = {'byr': checkBirthYear, 'iyr': checkIssueYear, 'eyr': checkExpirationYear,
                      'hgt': checkHeight, 'hcl': checkHairColor, 'ecl': checkEyeColor, 'pid': checkPassportID}
...
def checkHeight(checkValue):
    ...
    if match != None:
        if allowedValues[match.group(2)][0] <= int(match.group(1)) <= allowedValues[match.group(2)][1]:
            valuePassed = True
